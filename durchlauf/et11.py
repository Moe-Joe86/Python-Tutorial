# Etappe 11: Vererbung, und die Frage, ob wir sie brauchen (11a Gegner-Liste, 11b Trupp, 11c Items)
# Namen: durchgehend deutsch, ohne Umlaute
#
# 11a: gegner/gegner_typen (zwei parallele Listen) werden zu einer Liste von Gegner-Objekten.
# HINWEIS zur Benennung: Das Attribut heisst "entfernung", wie vom Guide vorgegeben. In meinem
# Modell seit Etappe 4 waechst die Position von 0 (Spawn) auf FELDER (Tor) - eine STEIGENDE Zahl.
# BOGEN.md beschreibt "entfernung" aber ausdruecklich als "eine sinkende Zahl" (Countdown zum Tor).
# Das ist ein Namens-/Semantik-Widerspruch zwischen dem, was Etappe 4 offen liess ("wo sie
# starten, entscheidest du"), und dem, was der Name in Etappe 11 nahelegt (siehe BERICHT.md).
# Ich behalte mein wachsendes Modell (durchgetestet seit Etappe 4) und uebernehme nur den Namen.
#
# 11b: Marine erbt von Einheit, vier Unterklassen (Soldat/Heavy/Engineer/Medic) ueberschreiben
# faehigkeit_einsetzen(). self.klasse als String entfaellt - type(self).__name__ ersetzt es
# (die Klasse selbst traegt die Information jetzt, keine doppelte Wahrheit mehr).
# Der Trupp besteht aus vier Marine-Objekten; nur der gesteuerte reagiert auf Eingaben (Etappe 12
# gibt den anderen einen Tick).
#
# 11c: Inventar.gegenstaende sind jetzt Item-Objekte statt Strings. Vergleiche laufen ueber
# .kennung, die Anzeige nimmt .name (Design-Entscheidung 1 aus Etappe 4 ist damit eingeloest).
#
# Charakterisierungstest (befehle.txt/diff gegen et10.py, PYTHONHASHSEED fixiert) fuer 11a,
# von Hand geprueft fuer 11b/11c (diff darf laut Guide hier abweichen). Siehe BERICHT.md.
...
# --- 11b: Einheit als gemeinsame Basis von Marine und Gegner ---

class Einheit:
    """Alles, was Trefferpunkte hat und Schaden austeilen kann - Marine und Gegner gemeinsam."""
    ...
    def nimm_schaden(self, menge):
        """Zieht Trefferpunkte ab. Gibt zurueck, ob die Einheit noch lebt."""
...
class Gegner(Einheit):
    ...
    Auftragsschritt 9: zwei Attribute fuer dieselbe Sache waeren zwei Wahrheiten)."""
...
def verarbeite_befehl(eingabe, marine, trupp, kern_integritaet, gegner, sektoren,
                       vorfeld, freigeschaltet, gesehene_gegnertypen):
    """
    Verarbeitet eine Eingabezeile fuer den gesteuerten Marine. Veraendert marine/gegner/vorfeld/
    freigeschaltet direkt (mutable). Gibt zurueck: rundenkosten, welle_beendet, eingabe_leer
    """
    ...
    if befehl == "status":
        ...
        for kamerad in trupp:
            ...
            print(f"  {kamerad.name} - {type(kamerad).__name__} - {kamerad.trefferpunkte} TP{markierung}")
        ...
        for i in range(schuesse):
            if marine.geladen > 0 and len(gegner) > 0:
                ...
                vorderster = gegner[0]
                for g in gegner:
                    if g.entfernung > vorderster.entfernung:
                        vorderster = g
                gegner.remove(vorderster)
                marine.berechne_schaden()  # bekommt nur einen Ort - Ausgabe bleibt unveraendert
                marine.erfahrung += 10
                marine.berechne_stufe()
                abgegeben += 1
                print(f"Treffer: {vorderster.name}. Geladen: {marine.geladen}, Gegner uebrig: {len(gegner)}")
        if abgegeben == 0:
...
if not klasse_gueltig:
    ...
    # Der Trupp: die drei anderen Klassen kommen automatisch dazu, mit eigenem Vorrat.
    trupp = [marine]
    for zahl, Klassenbauplan in KLASSEN_NACH_ZAHL.items():
        if Klassenbauplan is not GewaehlteKlasse:
            ...
            trupp.append(Klassenbauplan(kamerad_name, "nordtor", kamerad_vorrat, gesteuert=False))
    ...
    for i in range(welle):
        if welle <= 3:
            ...
            if i == 0:
                ...
                gegner_typ = "kriecher"
        gegner.append(Gegner(gegner_typ, trefferpunkte=20, schaden=5, entfernung=0))

    while True:
        eingabe = input(f"Welle {welle}, Runde {runde} > ")

        rundenkosten, welle_beendet, eingabe_leer = verarbeite_befehl(
            eingabe, marine, trupp, kern_integritaet, gegner, sektoren,
            vorfeld, freigeschaltet, gesehene_gegnertypen,
        )
