# Etappe 14: Das Vorfeld (14a Raster, 14b Reichweite und Bewegung; 14c Kuer, ausgelassen)
#
# 14a: Aus Gegner.entfernung (eine Zahl) werden x/y (eine Koordinate). welt.vorfeld ist
# eine Liste von Listen (welt.vorfeld[y][x] - Zeile vor Spalte, siehe Konzept 2). Riegel 2:
# Das Raster haelt nur Gelaende, keine Einheiten - die haben eigene x/y.
#
# Bewegungsentscheidung (Auftragsschritt 5, siehe GELERNT.md): ein Schritt pro Tick, eine
# Achse. Bei Gleichstand (|dx| == |dy|): zuerst x. Willkuerlich, aber festgelegt und ab
# jetzt ueberall gleich angewendet (Gegner UND Kameraden).
#
# 14b: welt.abstand() ist Manhattan-Distanz (abs(dx)+abs(dy)) - passt zur Ein-Achse-Bewegung
# aus 14a (Abstand = Anzahl Ticks bis dahin, siehe Konzept 9). welt.felder_in_reichweite()
# liefert ein Set von Tupeln. naechster_gegner() braucht jetzt einen Startpunkt (x,y), weil
# "am naechsten" vom Standort abhaengt. Trupp-KI Stufe 2: ausser Reichweite -> ein Schritt
# Richtung Ziel, nur wenn frei UND in der eigenen Zone.
#
# 14c (Barrikade) ausgelassen - laut Guide ausdruecklich Kuer ("wenn 14b sich zieht, lass
# 14c ganz weg"). Als offener Posten in GELERNT.md vermerkt.
#
# Aus 13b unveraendert: ausfallzeit (Einheit-Attribut, generisch fuer jede Einheit mit
# Ausfall/Respawn), magazin/nachladezeit fuer Kameraden, Basisturm (erbt Einheit, in
# welt.trupp UND unter welt.turm), welt.raeume_frei() setzt
# sektoren["osttor"]["nachbarn"]["osten"] (eigene Entscheidung aus Etappe 5).
#
...
def zeichne_vorfeld(vorfeld, trupp, gegner):
    ...
    Veraendert nichts, gibt nur aus."""
    bild = []
    ...
    for welle in range(1, wellen_bis_evakuierung + 1):
        ...
        while True:
            ...
            if not eingabe_leer:
                ...
                zeichne_vorfeld(welt.vorfeld, welt.trupp, welt.gegner)
