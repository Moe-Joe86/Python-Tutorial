...
## Nach dem Schreiben: Bogen **und** Lehrplan pflegen

**Ohne diese beiden Schritte ist die Etappe nicht fertig.** Es sind zwei Dateien, nicht eine — der Lehrplan wird regelmäßig vergessen, weil der Bogen mehr Aufmerksamkeit verlangt.
...
`RPG_Lehrplan.md` ist die Landkarte, und sie muss zum Gelände passen. Beim Ausformulieren eines Guides entstehen fast immer Dinge, die im Kurzabschnitt fehlen — und sie fehlen dort dauerhaft, wenn sie nicht sofort nachgetragen werden.

**Geh diese fünf Punkte durch. Jeder davon ist in der Praxis schon auseinandergelaufen:**

1. **Fehlende Themen.** Behandelt der Guide etwas, das im Kurzabschnitt nicht steht? Dann knapp ergänzen — ein bis drei Sätze, keine Wiederholung des Guides.
2. **Widersprüche.** Nennt der Lehrplan ein Beispiel, das der Guide anders löst? *(Vorgekommen: Der Lehrplan zeigte `startposition = (4, 7)` als Tuple-Beispiel, der Guide benutzt `RICHTUNGEN` — weil es vor Etappe 14 keine Koordinaten gibt.)*
3. **Fertiger Spielcode.** Steht im Kurzabschnitt Code, den der Lernende selbst schreiben soll? Der Lehrplan wird der begleitenden KI übergeben — dort ist eine Lösung genauso schädlich wie im Guide. *(Vorgekommen: Etappe 1 enthielt vier fertige `print`-Zeilen.)*
4. **Lernziele und Aufgaben.** Passen die Kurzfassungen im Lehrplan noch zu denen im Guide? Verlangt eine Transferaufgabe ein Werkzeug, das an dieser Stelle noch nicht da ist? *(Vorgekommen: Drei Transferaufgaben vor Etappe 7 verlangten eine Funktion.)*
5. **Zeitrahmen.** Wenn mehrere Guides eines Blocks fertig sind, ihre Sitzungszahlen addieren und die Blocktabelle daran eichen, statt zu schätzen.

**Und wenn sich die Nummerierung ändert:** Alle Guides, der Bogen, die README und die Navigationszeilen („Etappe N von 30") müssen mit. Das ist mechanisch, aber es wird übersehen.

Zur Kontrolle:

```bash
sed -n '/## Etappe N —/,/## Etappe N+1 —/p' RPG_Lehrplan.md    # Kurzabschnitt lesen
grep -c "von 30" de/*.md                                        # Navigation konsistent?
```

**Die Rollenverteilung, damit der Lehrplan nicht zum zweiten Guide wird:** Er *beschreibt*, er *lehrt nicht*. Was gebaut wird, warum es zählt, die Lernziele als Stichworte, ein Satz zur Transferaufgabe und zum Kaputtmachen. Erklärungen, Konzeptabschnitte und Experimente gehören in den Guide.
...
- [ ] Der Lehrplan-Abschnitt nennt alle Themen, die der Guide behandelt
- [ ] Lehrplan und Guide widersprechen sich nirgends (Beispiele, Lernziele, Aufgaben)
- [ ] Im Lehrplan steht kein Code, den der Lernende selbst schreiben soll
- [ ] Keine Aufgabe im Lehrplan verlangt ein Werkzeug aus einer späteren Etappe
