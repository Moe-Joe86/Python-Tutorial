# Etappe 9: Alles wird zum Objekt (9a Klassen/self, 9b __repr__)
...
# Charakterisierungstest (befehle.txt/diff gegen et8.py) wurde durchgefuehrt, siehe BERICHT.md.
...
class Marine:
    ...
    def __init__(self, name, klasse, klassengeraet, trefferpunkte, panzerung, schaden, sektor,
                 vorrat, inventar, magazin_groesse, geladen):
        self.name = name
        self.klasse = klasse
    ...
    def kaufe(self, ziel, menge_text):
        ...
        if gesamtpreis > self.vorrat["vaporium"]:
            ...
            return
        if ziel not in STAPELBAR and len(self.inventar) >= 10:
...
def verarbeite_befehl(eingabe, marine, kern_integritaet, gegner, gegner_typen, sektoren,
    ...
    if befehl == "status":
        ...
        if len(marine.inventar) == 0:
            print("Das Inventar ist leer.")
        else:
            namen = []
            for ding in marine.inventar:
        ...
        elif len(marine.inventar) >= 10:
            print("Das Inventar ist voll.")
        else:
            vorfeld.remove(ziel)
            marine.inventar.append(ziel)
            print(f"{ANZEIGENAMEN.get(ziel, ziel)} aufgenommen.")

    elif befehl == "ablege":
        if ziel == "":
            print("Ablegen was?")
        elif ziel not in marine.inventar:
            print(f"Du traegst kein {ANZEIGENAMEN.get(ziel, ziel)}.")
        else:
            marine.inventar.remove(ziel)
            vorfeld.append(ziel)
